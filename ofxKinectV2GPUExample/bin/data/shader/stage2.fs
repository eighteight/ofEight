...
void main(void)
{
  ...
  vec3 a = texelFetch2DRect(A, uv).xyz;
  vec3 b = texelFetch2DRect(B, uv).xyz;
  
  vec3 phase = atan(b, a);
    vec3 lt = vec3(lessThan(phase, vec3(0.0)));
  phase = mix(phase, phase + 2.0 * M_PI, lt);
//  phase = mix(phase, vec3(0.0), isnan(phase));
  ...
  float zmultiplier = texelFetch2DRect(ZTable, uv).x;
  float xmultiplier = texelFetch2DRect(XTable, uv).x;
  ...
  vec2 DepthAndIrSum = vec2(Depth, ir_sum);
