...
//	isSendingTUIO = true;
	...
	*/
	...
	thermitrackCam.sendStartCommand();
        ...
        vector <ofxThermitrackPoint> trackPnts;// = thermitrackCam.getTrackPoints();
			...
			if (currentPoints.size()>0){
				...
				for (int i = 0; i < delPoints.size(); i++){
					...
					ofVec2f sentPoint = rotatePoint(point);
...
//						if (cursor != NULL){
//							ofVec2f sentPoint = rotatePoint(trackPnts[i]);
...
bool testApp::canSendNextSignal(ofxThermitrackPoint tp){
	ofVec2f delta;// = tp.getCurrentDelta();
...
inline ofVec2f testApp::rotatePoint(ofxThermitrackPoint point){
		//don't rotate
...
void testApp::draw(){
    ...
    drawPoints(thermitrackCam.trackedPoints);
	drawInfo();
}
...
void testApp::drawPoints(vector <ofxThermitrackPoint> trackPnts){
	...
	for (int i = 0; i< trackPnts.size();i++){
		...
		ofVec2f rotatedPoint = rotatePoint(point);
...
//	string broadStatus = isSendingTUIO ? "Yes":"No";
	ofDrawBitmapString("Cam:"+thermitrackCam.getVerboseStatus()+"   Port:"+comPort+"   FPS:"+ofToString((int)ofGetFrameRate()), 20, 410);
...
void testApp::drawCam(){
	int * camPixelVals = thermitrackCam.camPixelVals;
