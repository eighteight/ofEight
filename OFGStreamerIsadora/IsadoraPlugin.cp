	...
	ActorInfo*			ioActorInfo)		// pointer to this actor's ActorInfo struct - unique to each instance of an actor
{
    ...
    guint flags;
    gst_init(NULL,NULL);
    memset (&info->mData, 0, sizeof (info->mData));
    ...
    /* Build the pipeline */
     	GError * error = NULL;
    info->mData.pipeline = gst_parse_launch ("playbin uri=http://docs.gstreamer.com/media/sintel_trailer-480p.webm", &error);
    //info->mData.pipeline = gst_parse_launch ("playbin uri=rtsp://10.0.1.12:8554", &error);
    
    if (error!=NULL) std::cout<<error->message<<std::endl;
    info->mBus = gst_element_get_bus (info->mData.pipeline);
    
    /* Set the download flag */
    error = NULL;
    g_object_get ( info->mData.pipeline, "flags", &flags, error);
    if (error!=NULL) std::cout<<error->message<<std::endl;
    flags |= GST_PLAY_FLAG_DOWNLOAD;

    error = NULL;
    g_object_set ( info->mData.pipeline, "flags", flags, error);
    if (error!=NULL) std::cout<<error->message<<std::endl;
    ...
    /* Start playing */
    ret = gst_element_set_state ( info->mData.pipeline, GST_STATE_PLAYING);
    if (ret == GST_STATE_CHANGE_FAILURE) {
        g_printerr ("Unable to set the pipeline to the playing state.\n");
        gst_object_unref ( info->mData.pipeline);
        //return -1;
    } else if (ret == GST_STATE_CHANGE_NO_PREROLL) {
        info->mData.is_live = TRUE;
    }
}
	...
	ActorInfo*			ioActorInfo)		// pointer to this actor's ActorInfo struct - unique to each instance of an actor
{
    ...
    gst_object_unref (info->mBus);
    gst_element_set_state ( info->mData.pipeline, GST_STATE_NULL);
    gst_object_unref ( info->mData.pipeline);
}
	...
	Boolean				inActivate)			// true when actor is becoming active, false otherwise.
{
	...
	if (inActivate) {
		...
		if (info->mBypass == false) {
            ...
            //gstreamer
            info->mData.loop = g_main_loop_new (NULL, FALSE);
//            info->mData.loop = info->loop;
//            info->mData.pipeline = info->pipeline;
            
            gst_bus_add_signal_watch(info->mBus);
            g_signal_connect (info->mBus, "message", G_CALLBACK (cb_message), &info->mData);
		}
